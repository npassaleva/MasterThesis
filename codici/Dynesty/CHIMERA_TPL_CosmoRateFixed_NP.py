...
if oph:
    ...
    dir_parent     = dir_data+"MICEv2_1.6M_L-compl_UCV.h5"
    dir_parent_int = dir_data+"p_bkg_gauss_smooth_zerr0.001_zres5000_smooth30.pkl"
    dir_out        = "/Users/niccolopassaleva/Desktop/Tesi/codici/"

    user, run, chain_letter, restart, subscript_to_restart, sampler = "NP", 1, "a", False, "1a", "dynesty"
...
from CHIMERA.astro.mass import pdf_PL
...
def generate_chain_filename(user, run, letter, restart = restart, subscript_to_restart = subscript_to_restart):
    ...
    if recipe["todo_mock"]=="O5_v3":
        if not restart:
            filename = f'chain_{user}_{str(run-1)}{letter}_TPL_CosmoRateFixed_O5.h5'
            ...
            filename = f'chain_{user}_{str(int(run_to_restart)+1)}{letter_to_restart}_TPL_CosmoRateFixed_O5.h5'

    if recipe["todo_mock"]=="O4_v3":
        if not restart:
            filename = f'chain_{user}_{str(run-1)}{letter}_TPL_CosmoRateFixed_O4.h5'
            
        else:
            run_to_restart    = ''.join(filter(str.isdigit, subscript_to_restart))
            letter_to_restart = ''.join(filter(str.isalpha, subscript_to_restart))
            filename = f'chain_{user}_{str(int(run_to_restart)+1)}{letter_to_restart}_TPL_CosmoRateFixed_O4.h5'
...
def generate_plots_filename(user, run, letter):
    ...
    if recipe["todo_mock"]=="O5_v3":
        plotname = f'chain_{user}_{str(run)}{letter}_TPL_CosmoRateFixed_O5'

    if recipe["todo_mock"]=="O4_v3":
        plotname = f'chain_{user}_{str(run)}{letter}_TPL_CosmoRateFixed_O4'
...
def generate_dynestysave_name(user, run, letter):
    ...
    if recipe["todo_mock"]=="O5_v3":
        dynestysave = f'dynesty_{user}_{str(run)}{letter}_TPL_CosmoRateFixed_O5.save'

    if recipe["todo_mock"]=="O4_v3":
        dynestysave = f'dynesty_{user}_{str(run)}{letter}_TPL_CosmoRateFixed_O4.save'
...
sampling(sampler, list(lambda_true.values()), recipe["nwalkers"])
