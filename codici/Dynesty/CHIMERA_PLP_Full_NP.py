...
def log_prob_mass_cosmo(lambdas):
    ...
    lambda_cosmo = {k : parameters[k] for k in priors_c.keys()}
    lambda_mass  = {k : parameters[k] for k in priors_m.keys()}
    lambda_rate  = {k : parameters[k] for k in priors_r.keys()}
...
def generate_chain_filename(user, run, letter, restart = restart, subscript_to_restart = subscript_to_restart):
    ...
    if not restart:
        filename = f'chain_{user}_{str(run-1)}{letter}.h5'
        ...
        filename = f'chain_{user}_{str(int(run_to_restart)+1)}{letter_to_restart}.h5'
 
    return filename

def generate_plots_filename(user, run, letter):

    plotname = f'chain_{user}_{str(run)}{letter}'

    return plotname

def generate_dynestysave_name(user, run, letter):

    dynestysave = f'dynesty_{user}_{str(run)}{letter}.save'
