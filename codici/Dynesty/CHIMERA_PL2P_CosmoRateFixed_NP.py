...
from CHIMERA.astro.mass import pdf_PL2P
...
def generate_chain_filename(user, run, letter, restart = restart, subscript_to_restart = subscript_to_restart):
    ...
    if recipe["todo_mock"]=="O5_v3":
        if not restart:
            filename = f'chain_{user}_{str(run-1)}{letter}_PL2P_CosmoRateFixed_O5.h5'
            ...
            filename = f'chain_{user}_{str(int(run_to_restart)+1)}{letter_to_restart}_PL2P_CosmoRateFixed_O5.h5'

    if recipe["todo_mock"]=="O4_v3":
        if not restart:
            filename = f'chain_{user}_{str(run-1)}{letter}_PL2P_CosmoRateFixed_O4.h5'
            
        else:
            run_to_restart    = ''.join(filter(str.isdigit, subscript_to_restart))
            letter_to_restart = ''.join(filter(str.isalpha, subscript_to_restart))
            filename = f'chain_{user}_{str(int(run_to_restart)+1)}{letter_to_restart}_PL2P_CosmoRateFixed_O4.h5'
...
def generate_plots_filename(user, run, letter):
    ...
    if recipe["todo_mock"]=="O5_v3":
        plotname = f'chain_{user}_{str(run)}{letter}_PL2P_CosmoRateFixed_O5'

    if recipe["todo_mock"]=="O4_v3":
        plotname = f'chain_{user}_{str(run)}{letter}_PL2P_CosmoRateFixed_O4'
...
def generate_dynestysave_name(user, run, letter):
    ...
    if recipe["todo_mock"]=="O5_v3":
        dynestysave = f'dynesty_{user}_{str(run)}{letter}_PL2P_CosmoRateFixed_O5.save'

    if recipe["todo_mock"]=="O4_v3":
        dynestysave = f'dynesty_{user}_{str(run)}{letter}_PL2P_CosmoRateFixed_O4.save'
