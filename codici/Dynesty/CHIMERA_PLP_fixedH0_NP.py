...
if oph:
    ...
    dir_parent     = dir_data+"MICEv2_1.6M_L-compl_UCV.h5"
    dir_parent_int = dir_data+"p_bkg_gauss_smooth_zerr0.001_zres5000_smooth30.pkl"
    dir_out        = "/Users/niccolopassaleva/Desktop/Tesi/chains/"

    user, run, chain_letter, restart, subscript_to_restart, sampler = "NP", 2, "a", False, "2a", "dynesty"
...
b     for i in range(initial_values.shape[0]):
...
def log_prob_mass_cosmo(lambdas):
    ...
    lambda_cosmo = {"H0": lambdas, "Om0": 0.3}
    lambda_mass  = lambda_mass_true
...
def generate_chain_filename(user, run, letter, restart = restart, subscript_to_restart = subscript_to_restart):
    ...
    if not restart:
        filename = f'chain_{user}_{str(run-1)}{letter}_H0fixed.h5'
        ...
        filename = f'chain_{user}_{str(int(run_to_restart)+1)}{letter_to_restart}_H0fixed.h5'
 
    return filename

def generate_plots_filename(user, run, letter):

    plotname = f'chain_{user}_{str(run)}{letter}_H0fixed'

    return plotname

def generate_dynestysave_name(user, run, letter):

    dynestysave = f'dynesty_{user}_{str(run)}{letter}_H0fixed.save'
